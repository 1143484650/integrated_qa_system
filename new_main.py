...
from rag_qa.core.qa_graph import QAGraphWorkflow
...
class IntegratedQASystem:
    def __init__(self):
        ...
        self.qa_graph = QAGraphWorkflow(self)
        self.last_results = {}
        # 初始化对话历史表，用于存储会话记录
    ...
    def query_result(self, query, source_filter=None, session_id=None):
        """执行 LangGraph 编排后的完整问答，返回结构化结果。"""
        self.logger.info(f"处理查询: '{query}' (会话ID: {session_id})")
        history = self.get_session_history(session_id) if session_id else []
        result = self.qa_graph.run(
            query=query,
        ...
        if session_id:
            ...
            self.last_results[session_id] = self._result_to_dict(result)
        self.logger.info(f"查询处理完成，trace_id={result.trace_id}, 耗时 {result.processing_time:.2f}秒")
        return result

    def get_last_result(self, session_id):
        return self.last_results.get(session_id, {})

    def retrieval_debug(self, query, source_filter=None, session_id=None):
        history = self.get_session_history(session_id) if session_id else []
        return self.qa_graph.debug_retrieval(
            query=query,
            source_filter=source_filter,
            history=history,
        )
    ...
    def query(self, query, source_filter=None, session_id=None):
        """查询集成系统，兼容原有 token 流式输出协议。"""
        result = self.query_result(query, source_filter=source_filter, session_id=session_id)
        answer = result.answer or "未找到答案"
        if result.answer_type in {"faq", "out_of_scope", "no_evidence"}:
            yield answer, True
            return
        for index in range(0, len(answer), 16):
            yield answer[index:index + 16], False
        yield "", True
