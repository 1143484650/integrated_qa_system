    ...
    def generate_response():
        try:
            ...
            for token, is_complete in qa_system.query(
                ...
                if is_complete:
                    message["metadata"] = qa_system.get_last_result(session_id)
                # 使用 SSE 格式：data: {json}\n\n
...
async def retrieval_debug(request: Request):
    try:
        body = await request.json()
    except Exception:
        raise HTTPException(status_code=400, detail="无效的 JSON 数据")
    ...
    if not query:
        raise HTTPException(status_code=400, detail="查询内容不能为空")
    return qa_system.retrieval_debug(query, source_filter=source_filter, session_id=session_id)
